  ...
  $.ajax({
    ...
    success: (response) => {
      ...
      for (var i = 0; i < restaurentData.length; i++) {
        restaurentData[i].starTemplate = getRatingStarTemplate(
          restaurentData[i].ratings
        );
        $("#exploreRestaurants").append(
          singleRestaurentTemplate(restaurentData[i])
        );
      }
    },
    error: (err) => {
